...
```{r read_data}
station<-read.csv('H:/fun data/indego/updated_station_info.csv', header = TRUE)
weather<-read.csv('H:/fun data/indego/may2016toapr2019weather.csv', header = TRUE)
emily_bike_raw<-read.csv('H:/fun data/indego/personal test data.csv', header = TRUE)
...
all_stations <- start_stations %>% 
 ...
 # dplyr::select(-relative_n) %>% 
  ...
  dplyr::mutate(relative_start_n = min(start*0.7,41),
                relative_end_n = min(end*0.7,41),
...
selectInput("stationselect", label = NULL, #h5("Select a Start Station:"),
            choices = station_names, selected = "20th & Fairmount")
```
...
### **Number of Rides Taken <br>May 2016 - April 2019** 
...
renderValueBox ({
  valueBox(
    ...
    icon ="fa-bicycle", 
    color = "#16216A") #indego navy;old blue = #5d98f7
})
```

### **Number of Miles Biked <br>May 2016 - April 2019** 

```{r num_miles}
 
# Creating the value we want to display
num_miles<-sum(emily_data_clean$miles_clean)
...
### **Overall Cost per Ride <br>May 2016 - April 2019** 

```{r money}
 
# Creating the value we want to display # cost through end of May 2019
cost<-paste0('$',round((566-9)/num_rides,2))
